...
def run_test_better_draw_circles():
    print()
    print('--------------------------------------------------')
    print('Testing the   better_draw_circles   function:')
    print('--------------------------------------------------')
    better_draw_cirles(10)


def better_draw_cirles(n):
    window = rg.RoseWindow(400, 400)
    center = rg.Point(200, 200)
    for k in range(21):
...
def run_test_even_better_draw_circles():
    print()
    print('--------------------------------------------------')
    print('Testing the   even_better_draw_circles   function:')
    print('--------------------------------------------------')
    window_wise=400
    window_high=400
    center_of_the_circle_x=200
    center_of_the_circle_y=200
    the_number_of_circle_draw=30
    the_amount_by_which_the_radii_vary=5
    time=0.1
    color='blue'
    even_better_draw_circles(window_wise,window_high,center_of_the_circle_x,
                       center_of_the_circle_y,the_number_of_circle_draw,
                       the_amount_by_which_the_radii_vary,time,color)


def even_better_draw_circles(win_x,win_y,cen_x,cen_y,num_of_cir,n,t,color):
    window = rg.RoseWindow(win_x, win_y)
    center = rg.Point(cen_x, cen_y)
    for k in range(num_of_cir+1):
        circle = rg.Circle(center, n * (k))
        circle.attach_to(window)
